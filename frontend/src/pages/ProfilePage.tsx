...
export function ProfilePage() {
    ...
    const fetchInventory = useCallback(async () => {
        ...
        try {
            ...
            if (
                !ownershipsResponse.success ||
                !ownershipsResponse.responseObject
            ) {
                setError(
                    ownershipsResponse.message ||
            ...
            const giftsWithDetails = await Promise.all(
                ownerships.map(async (ownership) => {
                    try {
                        ...
                        const giftResponse = await getGiftById(
                            ownership.gift_id
                        );
                        if (
                            giftResponse.success &&
                            giftResponse.responseObject
                        ) {
                            ...
                            try {
                                const collectionResponse =
                                    await getCollectionById(gift.collection_id);
                                if (
            ...
            const validGifts = giftsWithDetails.filter(
                (gift): gift is NonNullable<typeof gift> => gift !== null
            );
                                            ...
                                            <div className="pt-3 border-t space-y-2">
                                                <div className="flex justify-between items-center text-sm">
                                                    ...
                                                    <span className="text-primary font-medium">
                                                        {gift.ownership.acquired_price.toFixed(
                                                            2
                                                        )}
                                                    </span>
                                                </div>
